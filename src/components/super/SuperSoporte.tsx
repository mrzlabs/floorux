...
interface SuperSoporteProps {
  profile: Profile;
}

export function SuperSoporte({ profile }: SuperSoporteProps) {
  ...
  // super_root ID = sender of any message that isn't mine
  const superRootId = thread.find(m => m.sender_id !== profile.id)?.sender_id ?? null;
  ...
  async function handleSend() {
    ...
    if (isNewTicket) {
      ...
      if (ok) toast('Solicitud enviada a Super Root', 'check');
    } else {
      if (!superRootId) { toast('Crea una solicitud primero para iniciar la conversación', 'alert'); setSending(false); return; }
      ok = await sendReply(superRootId, body.trim());
    }
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.4fr', gap: 16, alignItems: 'start' }}>
      ...
      <div className="card" style={{ padding: 20 }}>
        <h2 style={{ fontSize: 16, fontWeight: 800, marginBottom: 16 }}>
          {thread.length > 0 ? 'Nueva solicitud o respuesta' : 'Pedir soporte a Super Root'}
        </h2>

        <Field label="Asunto (deja vacío para responder)">
          <input className="inp" value={asunto} placeholder="Ej. Problema de acceso, Facturación…"
        ...
        <Field label={isNewTicket ? 'Detalle' : 'Mensaje'}>
          <textarea className="inp" rows={6} value={body}
            placeholder={thread.length > 0 ? 'Responde o detalla tu nueva solicitud…' : 'Describe tu problema o consulta…'}
            onChange={e => setBody(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter' && e.ctrlKey) handleSend(); }}
          />
        </Field>
        <p className="muted" style={{ fontSize: 11, marginBottom: 10 }}>Ctrl+Enter para enviar</p>

        <button className="btn pri block" onClick={handleSend}
          disabled={sending || !body.trim() || (!isNewTicket && !superRootId)}>
          <Icon name="send" s={15} />
      ...
      <div className="card" style={{ padding: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <h2 style={{ fontSize: 16, fontWeight: 800 }}>Conversación con Super Root</h2>
          {latestStatus && (
        ...
        {thread.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '32px 0' }}>
            ...
            <p className="muted" style={{ fontSize: 13, marginTop: 10 }}>
              Aún no has enviado ninguna solicitud.<br />Usa el formulario para contactar a Super Root.
            </p>
          </div>
        ) : (
